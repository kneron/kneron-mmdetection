...
try:
    import kp
    from kp import inference as kip
except ImportError:
    warnings.warn(
        "Kneron PLUS software failed to import; please see "
        "document http://doc.kneron.com/docs/#plus_python "
    ...
    def forward_kneron(self,
        ...
        if hasattr(self, '__Kn_ONNX_Sess__'):
            tmp = getattr(self, '__Kn_ONNX_Sess__').run(
                None, {'input': img.cpu().detach().numpy()}
            )
            device = torch.device(
            ...
            assert hasattr(
                self.bbox_head, 'get_bboxes_kn'
            ), 'Error: None implemented kneron bbox_head forward type!'
            ...
            try:
                kip.generic_raw_inference_send(
                    device_group=kp_params['device_group'],
                    generic_raw_image_header=kp_params[
                ...
                generic_raw_res = kip.generic_raw_inference_receive(
                    device_group=kp_params['device_group'],
                    generic_raw_image_header=kp_params[
                        'generic_raw_image_header'],
                ...
                for node_idx in range(generic_raw_res.header.num_output_node):
                    float_res = kip.generic_inference_retrieve_float_node(
                        node_idx=node_idx,
                        generic_raw_result=generic_raw_res,
                        channels_ordering=(
                            kp.ChannelOrdering.KP_CHANNEL_ORDERING_CHW
                        )
                    )
                    inf_node_output_list.append(
                        float_res.ndarray.copy()
                    )
            except Exception as e:
                raise Exception(
                    e,
                    'Kneron PLUS software have something wrong. '
                    'If Kneron PLUS software haven\'t been installed, '
                    'please check the document: '
                    'http://doc.kneron.com/docs/#plus_python/. '
                    'If you found something unexpected, '
                    'welcome to give us some feedbacks in our forum '
                    'https://www.kneron.com/forum to help us make it better!',
                )
